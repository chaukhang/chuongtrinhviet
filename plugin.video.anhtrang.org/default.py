...
mysettings = xbmcaddon.Addon(id='plugin.video.anhtrang.org')
...
def addDir(name,url,mode,iconimage,edit,inum):
    ...
    contextmenuitems = []
    if edit:
        contextmenuitems.append(('[COLOR red]Delete[/COLOR] [COLOR ffff8c00]Search[/COLOR]','XBMC.Container.Update(%s?url=%s&mode=8&name=%s&inum=%d)'%('plugin://plugin.video.anhtrang.org',urllib.quote_plus(url),urllib.quote_plus(name),inum)))
        contextmenuitems.append(('[COLOR yellow]Edit[/COLOR] [COLOR ffff8c00]Search[/COLOR]','XBMC.Container.Update(%s?url=%s&mode=9&name=%s&inum=%d)'%('plugin://plugin.video.anhtrang.org',urllib.quote_plus(url),urllib.quote_plus(name),inum)))
